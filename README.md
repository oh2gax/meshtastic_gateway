...
All received direct messages with date/time, RSSI/SNR, and per-message actions. The *From* cell is rendered on a single compact line: the resolved node name (long + short), the node id (only shown when a name was actually received, so we don't duplicate id with id), an optional `· Hops N` annotation showing how many hops *that specific message* travelled (`0` = direct neighbor, omitted entirely if hop info wasn't in the packet), and the `· Chat · Reply` action links — all on one row.
...
All sent direct messages, their destination, and an **Ack** column showing the delivery state of each outgoing message. The *To* cell is rendered compactly on a single line (name + id when available + `· Chat` link). The earlier *Status* column (`queued` / `sent` / `failed`) was removed because the Ack column conveys the meaningful per-message state more clearly and the freed horizontal space lets longer node names and message bodies render without wrapping. The display follows Android-style conventions:
...
### Chat — per-node DM thread

The *Chat* page shows the conversation with a single specific node, direct messages only — broadcast traffic that the node may also generate is filtered out. Reachable from the navbar *Chat* button (which lands on the picker with no conversation selected) or from any *Chat* link in the Inbox, Outbox, or Active Nodes lists.

- **Chronological layout.** Messages render oldest-first / newest-last, and the page auto-scrolls to the bottom on every load so you immediately see the latest exchange.
- **Node picker.** A dropdown at the top lists every node currently in the gateway's database (most-recent-first), with the gateway's own node excluded. Picking a different one navigates directly to that conversation.
- **Mark-as-read on open.** Opening a chat with a node marks all unread DMs from that node as read, so the navbar's *New message* indicator and the Inbox row highlight clear automatically.
- **Auto-refresh.** Same `Off / 10s / 30s / 60s` dropdown as Active Nodes / Broadcast / Outbox, with its own `localStorage` slot. Defaults to *Off*; set to 10 s while you're actively waiting for replies.
- **Send-and-stay.** The send form posts to a chat-specific endpoint, so after queuing a message you stay on the same chat rather than being redirected to the Outbox.
- **Per-message annotations.** Each outgoing row carries the same `Y / S / … / —` ACK indicator used in the Outbox (so you can see delivery status inline). Each incoming row carries a `· Hops N` annotation showing how many hops *that specific message* travelled (`0` = direct, omitted when the metadata isn't present).
...
Use the web UI's *Send* page for one-off messages, or the *Chat* page (via the navbar button or any per-row *Chat* link) for an ongoing thread with a specific node. ACK status appears in the Outbox column and inline next to each outgoing chat line.
